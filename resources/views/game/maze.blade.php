<script>
  ...
  if (BlocklyGames.LEVEL < BlocklyGames.MAX_LEVEL) {
    window.location = window.location.protocol + '//' +
        ...
        '?id=' + (BlocklyGames.LEVEL + 1) + '&level=' + (BlocklyGames.LEVEL + 1);
  } else {
    window.location = 'http://127.0.0.1:8000/ilhas';
  }
};
  
  Maze.MAX_BLOCKS = [undefined, // Level 0.
    Infinity, Infinity, 2, 5, 5, 6, 7, 13, 10, 11, 13, 13, 13, 13, 17][BlocklyGames.LEVEL];
  ...
  Maze.SKINS = [
    ...
    {
      sprite: '/storage/pegman/{{$task->pegman}}',
      tiles: '/storage/tiles/{{$task->tiles}}',
      marker: '/storage/levels/flags/{{$task->flags}}',
      background: '/storage/levels/{{$task->background}}',
      graph: false,
      look: '#000',
  ...
  Maze.SKIN_ID = Maze.SKINS[0];
  Maze.SKIN = Maze.SKIN_ID;
...
// Level 1.
 [{{$task->map}}]
  ...
  Maze.drawMap = function() {
    ...
    finishMarker.setAttribute('width', 71);
  ...
  Maze.init = function() {
    ...
    pegSpin.id = 'pegSpin';
    pegSpin.src = '/images/game/1x1.gif';
  ...
  Maze.levelHelp = function(opt_event) {
    ...
    if (BlocklyGames.LEVEL == 1) {
      ...
      if (!Maze.levelHelp.initialized7_) {
        ...
        var options =
            [BlocklyGames.Msg['Maze_pathAhead'],
             BlocklyGames.Msg['Maze_pathLeft'],
             BlocklyGames.Msg['Maze_pathRight']];
  ...
  Maze.updateCapacity = function() {
    ...
    if (cap == Infinity) {
      ...
      if (cap == 0) {
        var msg = BlocklyGames.Msg['Maze_capacity0'];
      } else if (cap == 1) {
        var msg = BlocklyGames.Msg['Maze_capacity1'];
      } else {
        var msg = BlocklyGames.Msg['Maze_capacity2'];
      }
      var parts = msg.split(/%\d/);
      for (var i = 0; i < parts.length; i++) {
 ...
 Maze.lines = function(){
  ...
  Maze.Complete = function(){
      $.ajax({
            type: "POST",
            url: "{{action('GameController@taskComplete')}}",
            data: {"id":"{{$task->id}}","nameLevel":"{{$task->name}}" ,"moves":Maze.lines()},
